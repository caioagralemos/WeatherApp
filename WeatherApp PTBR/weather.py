...
label = customtkinter.CTkLabel(master=frame, text="Escolha sua cidade:", font=("Helvetica Neue", 24))
label.pack(pady=12, padx=10)

entry = customtkinter.CTkEntry(master=frame, placeholder_text="Cidade", font=("Helvetica Neue", 12))
...
def weatherapp(cidade):
    ...
    ltemp = customtkinter.CTkLabel(window, text=f"Temperatura em {CITY}: {celsius}°C")
    ltemp.pack()

    lminmax = customtkinter.CTkLabel(window, text=f"Mínima: {min_celsius}°C Máxima: {max_celsius}°C")
    lminmax.pack()

    if (weather == "Clouds"):
        lweather = customtkinter.CTkLabel(window, text=f"Nublado, céu coberto")
        lweather.pack()
    elif (weather == "Mist"):
        lweather = customtkinter.CTkLabel(window, text=f"Neblina, {weather_description}")
        lweather.pack()
    elif (weather == "Rain"):
        lweather = customtkinter.CTkLabel(window, text=f"Chuva, chuva média")
        lweather.pack()
    elif (weather == "Snow"):
        lweather = customtkinter.CTkLabel(window, text=f"Neve, frio intenso")
        lweather.pack()
    elif (weather == "Clear"):
        lweather = customtkinter.CTkLabel(window, text=f"Sol, céu limpo")
        lweather.pack()
    elif (weather == "Thunderstorm"):
        lweather = customtkinter.CTkLabel(window, text=f"Tempestade, chuva forte")
        lweather.pack()
    else:
        lweather = customtkinter.CTkLabel(window, text=f"{weather}, {weather_description}")
        lweather.pack()

    lsun = customtkinter.CTkLabel(window, text=f"Nascer do sol: {sunrise_time}, Pôr-do-sol: {sunset_time}")
    lsun.pack()
...
button = customtkinter.CTkButton(master=frame, text="Enter", font=("Helvetica Neue", 12), command=get_data)
